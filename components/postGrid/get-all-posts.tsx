...
//components/postGrid/get-all-posts.tsx
...
import { FiUser, FiEye, FiClock, FiArrowRight, FiBook, FiHeart, FiFilter } from 'react-icons/fi'
...
const PostCards = ({ filters }: PostCardsProps) => {
    ...
    const fetchPosts = async () => {
      try {
        ...
        if (result.success && result.data) {
          ...
          // Load likes in background without blocking UI
  ...
  const filteredPosts = useMemo(() => {
    if (!filters || !allPosts.length) return allPosts;
    ...
    if (filters.author !== 'all') {
      filtered = filtered.filter(post => 
        post.author.name.toLowerCase() === filters.author.toLowerCase()
      );
    }
  ...
  // Extract unique categories and authors for filter dropdowns (from real data)
  const categories = useMemo(() => {
    const uniqueCategories = [...new Set(allPosts.map(post => post.category.name))];
    return uniqueCategories.sort();
  }, [allPosts]);

  const authors = useMemo(() => {
    const uniqueAuthors = [...new Set(allPosts.map(post => post.author.name))];
    ...
    <div className="min-h-screen bg-gray-50 dark:bg-gray-900 py-8">
      <div className="max-w-7xl mx-auto px-4">
        ...
        <div className="text-center mb-8">
          ...
          <div className="flex flex-col sm:flex-row items-center justify-center gap-4 text-xs text-gray-500 dark:text-gray-400 font-mono">
            <div>
              {posts.length} {posts.length === 1 ? 'POST' : 'POSTS'} DISPLAYED
            </div>
            {filters && (filters.searchQuery || filters.category !== 'all' || filters.author !== 'all') && (
